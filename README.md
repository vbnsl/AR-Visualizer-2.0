...
| **Tile render pipeline** | `lib/tiledWall.ts` — `renderTiledWall` | Order: pattern → lighting multiply (optional second pass if `lightingStrength` > 1) → noise → warp. Options: `lightingCanvas`, `lightingStrength`, `noiseOpacity`. |
| **Floor depth gradient** | `components/TileOverlayCanvas.tsx` — when `surface="floor"` | Linear multiply gradient (darker at quad “top” / far edge). Tune `FLOOR_DEPTH_GRADIENT_STRENGTH` (default 0.2). Set to 0 to disable. |
| **Floor lighting strength** | `lib/tiledWall.ts` + `TileOverlayCanvas.tsx` | When `surface="floor"`, lighting is applied with `lightingStrength` (default 1.2) for stronger room falloff. Tune `FLOOR_LIGHTING_STRENGTH` in `TileOverlayCanvas.tsx`. |
| **Floor atmospheric** | `components/TileOverlayCanvas.tsx` — when `surface="floor"` | Subtle desaturation toward far (top) edge via "color" blend with gray gradient. Tune `FLOOR_ATMOSPHERIC_STRENGTH` (default 0.12). Set to 0 to disable. |
| **Floor edge vignette** | `components/TileOverlayCanvas.tsx` — when `surface="floor"` | Subtle darkening toward quad edges (radial from center). Tune `FLOOR_EDGE_VIGNETTE` (default 0.08). Set to 0 to disable. |
| **Floor feather** | `components/TileOverlayCanvas.tsx` — when `surface="floor"` | Softer blend at quad edges; floor uses `FLOOR_FEATHER_PX` (default 8), wall uses `FEATHER_PX` (5). |
| **Floor noise** | `components/TileOverlayCanvas.tsx` — when `surface="floor"` | Slightly higher micro-noise for floor; tune `FLOOR_NOISE_OPACITY` (default 0.02). Wall uses `NOISE_OPACITY` (0.015). |

**Pipeline summary (TileOverlayCanvas):** (1) Occlusion source: depth → DeepLab → edge; smooth. (2) Combine feathered quad (floor uses FLOOR_FEATHER_PX) + occlusion (min alpha). (3) If occlusion exists, extract wall-only lighting. (4) Draw tiled wall to offscreen (floor: stronger lighting, FLOOR_NOISE_OPACITY). (5) **Floor only:** depth gradient (darker far edge). (6) **Floor only:** atmospheric desaturation (far edge more muted). (7) **Floor only:** edge vignette. (8) Destination-in with combined mask.

### Floor vs wall

- **Wall:** Single plane at one distance; same realism (feather, lighting, noise) applies.
- **Floor:** Recedes into the room, so we add: (1) **Stronger lighting** — `lightingStrength` (e.g. 1.2) in `renderTiledWall`; tune `FLOOR_LIGHTING_STRENGTH` in `TileOverlayCanvas.tsx`. (2) **Depth gradient** — the “far” (top) edge of the floor quad is darkened with a linear multiply so the floor reads with more depth; tune `FLOOR_DEPTH_GRADIENT_STRENGTH` (default 0.2). (3) **Atmospheric** — subtle desaturation toward the far edge; tune `FLOOR_ATMOSPHERIC_STRENGTH` (default 0.12). (4) **Edge vignette** — subtle darkening toward quad edges; tune `FLOOR_EDGE_VIGNETTE` (default 0.08). (5) **Feather** — floor uses `FLOOR_FEATHER_PX` (8) for softer edges. (6) **Noise** — floor uses `FLOOR_NOISE_OPACITY` (0.02). The page passes `surface="floor"` for the floor overlay and omits it (or `"wall"`) for the wall.
